...
function constructHofstadterVqLL(params::Params;p::Int=1,q::Int=16,nLL::Int=10,nγ::Int=2,lk::Int=10)
    ...
    A.spectrum = zeros(Float64,2A.q,A.l1,A.l2,length(A.ν))
    for ik2 in 1:A.l2, ik1 in 1:A.l1
        println(ik1," ",ik2)
        computeSingleParticleSpectrum(A,params,ik1,ik2)
        for iν in eachindex(A.ν)
            A.spectrum[:,ik1,ik2,iν] = eigvals(Hermitian(A.H[:,:,iν]))
        end
    end
...
@inline function Knm(n::Int,m::Int,qlB::ComplexF64)
    ...
    x = - 0.5 * abs2(qlB)
    if n>= m 
        return exp(x/2)*sqrt(gamma(m+1)/gamma(n+1)) * laguerrel(m,n-m,-x) * cplus^(n-m)
    else 
        return exp(x/2)*sqrt(gamma(n+1)/gamma(m+1)) * laguerrel(n,m-n,-x) * cminus^(m-n)
    end
end
...
function constructΛs(A::HofstadterVqLL,params::Params,s1::Int,m1::Int,ik1::Int,ik2::Int)
    Λw = reshape(A.Λw,A.nH,2,A.nH,2,A.l1,A.l2)
    Λw .= 0.0 + 0.0im
    #ik1,ik2  # external momenta
        ...
        for iH1 in 1:A.nH 
            ...
            for iH2 in 1:A.nH
                ...
                for ilayer in 1:2
                    ...
                    k2l = imag(A.kvec[ik1,ik2]) - imag(Kl)
    ...
    q = A.q
    nLL = A.nLL ÷q
    Λ0 = load("2piFlux/NarrowBandEigenstates_w0$(w0)_q$(q)_nLL$(nLL).jld","vec")  # (2nH,2q,l1,l2)
    Uort_k = Λ0[:,:,ik1,ik2]
    for ip2 in 1:A.l2, ip1 in 1:A.l1 
        Uort_p = Λ0[:,:,ip1,ip2]
...
function computeSingleParticleSpectrum(A::HofstadterVqLL,params::Params,ik1::Int,ik2::Int)
    A.H .= 0.0 + 0.0im
    for s1 in A.svec
        # println(s1)
        for m1 in A.nvec
            Vq = reshape( Coulomb.( A.kvec[ik1,ik2] .- A.kvec .- s1/A.q * params.g2 .- m1*params.g1  ), 1,1,1,:)
            constructΛs(A,params,s1,m1,ik1,ik2)
            ...
            is1 = s1 - A.svec[1] + 1
            im1 = m1 - A.nvec[1] + 1
            tmp = Coulomb( -s1/A.q*params.g2 - m1*params.g1 ) * (A.ρq[im1,is1] * A.ΛΨ[:,:,ik1,ik2]' )
            for iν in eachindex(A.ν)
                A.H[:,:,iν] .+= A.ν[iν] * tmp
            end
        end
