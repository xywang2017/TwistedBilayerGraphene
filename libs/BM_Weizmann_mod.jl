...
function ComputeH(H::Matrix{ComplexF64},blk::HBM,params::Params,k::ComplexF64;ig::Vector{Int}=[0,0])
    ...
    for ig in 1:blk.lg^2
        ...
        k1 = [real(kb);imag(kb)]
        k2 = [real(kt);imag(kt)]
        H[(4idg[ig]-3):(4idg[ig]-2),(4idg[ig]-3):(4idg[ig]-2)] = params.vf*dirac(k1[1]+1im*k1[2],0.0) .+ params.δ*σz
        H[(4idg[ig]-1):(4idg[ig]),(4idg[ig]-1):(4idg[ig])] = params.vf*dirac(k2[1]+1im*k2[2],0.0)
        # H[(4idg[ig]-3):(4idg[ig]-2),(4idg[ig]-3):(4idg[ig]-2)] = params.vf*diracvalleyKprime(k1[1]+1im*k1[2],0.0) .+ params.δ*σz
        # H[(4idg[ig]-1):(4idg[ig]),(4idg[ig]-1):(4idg[ig])] = params.vf*diracvalleyKprime(k2[1]+1im*k2[2],0.0)
    end
...
function initHBM(blk::HBM,Latt::Lattice,params::Params;lg::Int=9)
    ...
    for ik in eachindex(Latt.kvec)
        ...
        vals, vecs = eigs(H,nev=blk.nflat,which=:SM)
