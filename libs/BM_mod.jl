...
function generate_T12(blk::HBM,params::Params)
    ...
    for ig in eachindex(idg)
        tmp[3:4,idg[ig],1:2,idg_nn1[ig]] = params.T1
        tmp[1:2,idg_nn1[ig],3:4,idg[ig]] = params.T1

        tmp[3:4,idg[ig],1:2,idg_nn2[ig]] = params.T2
        tmp[1:2,idg_nn2[ig],3:4,idg[ig]] = params.T2
...
function ComputeH(H::Matrix{ComplexF64},blk::HBM,params::Params,k::ComplexF64;ig::Vector{Int}=[0,0])
    ...
    for ig in 1:blk.lg^2
        qc = blk.gvec[ig]
        H[(4idg[ig]-3):(4idg[ig]-2),(4idg[ig]-3):(4idg[ig]-2)] = params.vf*dirac(k+qc-params.Kb)
        H[(4idg[ig]-1):(4idg[ig]),(4idg[ig]-1):(4idg[ig])] = params.vf*dirac(k+qc-params.Kt)
    end
