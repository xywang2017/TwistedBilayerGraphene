...
function initHofstadterVq(A::HofstadterVq, params::Params;p::Int=1,q::Int=5,lk::Int=4,m::Int=1,n::Int=1,flagM::Bool=true,flagS::Bool=true)
    ...
    if flagM == true
        ...
        computeOrtNormMTG(A)
        save("StrongCoupling/Tempq$(q)/OverLapM.jld","M",hof.M,"Mz",hof.Mz,"Uort",hof.Uort,"Σz",hof.Σz)
    else
        data = load("StrongCoupling/Tempq$(q)/OverLapM.jld")
        A.M = data["M"]
        A.Mz = data["Mz"]
        A.Uort = data["Uort"]
        A.Σz = data["Σz"]
    ...
    if flagS == true
        computeSingleParticleSpectrum(A,m,n)
        Hq_re = real(A.H)
        Hq_im = imag(A.H)
        writedlm("StrongCoupling/Tempq$(q)/Hq_$(m)_$(n).txt",[Hq_re[:] Hq_im[:]])
    end
...
function computeSingleParticleSpectrum(A::HofstadterVq,m::Int,n::Int)
    # kvec in magnetic strip
    kvec = reshape(A.k1,:,1) * params.g1 .+ reshape(A.k2,1,:) * params.g2
    kext = [0.0 + 0.0im]
    A.H = zeros(ComplexF64,2A.q,2A.q,1)
    gq = real(A.gq) * A.g1 + imag(A.gq) * A.g2 / A.q
    A.OΨ = zeros(ComplexF64,2A.q,1,2A.q,A.l1*A.l2)

    ig = CartesianIndex(m,n)
    # println(ig[1]," ",ig[2])
    computeCoulombOverlap(A,ig[1],ig[2])

    Vq = reshape( Coulomb.( reshape(kext,:,1) .- reshape(kvec,1,:) .- gq[ig]), 
                (1,1,1,1,A.l1*A.l2) )
    OΨ1 = reshape(A.OΨ,2A.q,1,1,2A.q,A.l1*A.l2)
    OΨ2 = reshape(A.OΨ,1,2A.q,1,2A.q,A.l1*A.l2)
    A.H .= reshape( sum(Vq .* OΨ1 .* conj.(OΨ2), dims=(4,5)), (2A.q, 2A.q, 1))
...
function ConstructHBM_Hoftstadter(hof::HofstadterVq,params::Params;lg::Int=9)
    ...
    for ik in eachindex(kvec)
        ...
        if abs(vecs[:,1]' * vecs[:,2]) > 1e-6
            println("eigenvector not orthogonalized ")
        end
