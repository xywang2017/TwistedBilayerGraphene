...
mutable struct HofstadterVq 
    ...
    #
    H0::Array{ComplexF64,3} # zeros field Hamiltonian: 2x2xgrid
...
function initHofstadterVq(A::HofstadterVq, params::Params;p::Int=1,q::Int=5,lk::Int=4,nLL::Int=10)
...
function computeSingleParticleSpectrumB0(A::HofstadterVq)
    ...
    kvec = A.kvec
    A.H0 = zeros(ComplexF64,2,2,A.l1*A.l2*A.q)
    ...
    for ig in CartesianIndices(A.δg)
        ...
        A.H0 .+= reshape( sum(Vq .* Λ1 .* conj.(Λ2), dims=(4,5)), (2, 2, A.l1*A.l2*A.q))
    end
...
function computeOrtNormMTG(A::HofstadterVq)
    ...
    # if norm(A.Uort[:,:,1]'*A.Uort[:,:,1]-Diagonal(A.Uort[:,:,1]'*A.Uort[:,:,1]))  > 1e-6
    #     println("Error with Uort normalization ",norm(A.Uort[:,:,1]'*A.Uort[:,:,1]-I))
    # end
    return nothing
end
...
function computeSingleParticleSpectrum(A::HofstadterVq)
    ...
    for ig in CartesianIndices(A.gq)
        ...
        A.H .+= reshape( sum(Vq .* OΨ1 .* conj.(OΨ2), dims=(4,5)), (2A.q, 2A.q, A.l1*A.l2))
...
function computeSingleParticleSpectrum_hole(A::HofstadterVq)
    # kvec in magnetic strip
    kvec = reshape(A.k1,:,1) * params.g1 .+ reshape(A.k2,1,:) * params.g2
    A.H = zeros(ComplexF64,2A.q,2A.q,A.l1*A.l2)
    gq = real(A.gq) * A.g1 + imag(A.gq) * A.g2 / A.q
    ...
    for ig in CartesianIndices(A.gq)
        ...
        println(ig[1]," ",ig[2])
        gq = real(A.gq[ig]) * A.g1 + imag(A.gq[ig]) * A.g2/A.q
        Vq = reshape( Coulomb.( reshape(kvec,:,1) .- reshape(kvec,1,:) .- gq), (1,A.l1*A.l2,1,1,A.l1*A.l2) )

        computeCoulombOverlap(A,real(A.gq[ig]),imag(A.gq[ig]))
        OΨ1 = reshape(A.OΨ,2A.q,A.l1*A.l2,2A.q,1,A.l1*A.l2)
        OΨ2 = reshape(A.OΨ,2A.q,A.l1*A.l2,1,2A.q,A.l1*A.l2)
        A.H .+= reshape( sum(Vq .* OΨ1 .* conj.(OΨ2), dims=(1,2)), (2A.q, 2A.q, A.l1*A.l2))
...
@inline function ΨLL(n::Int,x::Float64) ::Float64 #Landau level wavefunction -dimensionless part
    if n<= 120
        return π^(1/4)* exp(-x^2/2) * hermiteh(n,x) / ( 2.0^(n/2-0.5) * sqrt( gamma(n+1.0) ) ) 
    else
        return π^(-1/4)/ sqrt( gamma(n+1.0) ) * 2.0^(n/2+0.5) * gamma((n+1.0)/2) * cos(x*sqrt(2n)-n*π/2)
    end
    # return π^(1/4)* exp(-x^2/2) * hermiteh(n,x) / ( 2.0^(n/2-0.5) * sqrt( factorial(big(n)) ) ) 
end
        ...
        # if abs(s1) < A.l1
            ...
            for ilayer in 1:2
                ...
                for z1 in 1:lg , ik1bar in eachindex(A.k1)
                    ...
                    θ3 = exp(-1im * kxbar * x0l) * exp(1im * x0l * real(Kl))
                    for iLL in 0:(A.nLL-1), iα in 1:2
                        xnum = (kxbar - real(Kl)) * lB
                        iH = indexLL(iα,iLL,ilayer)
                        if iLL == 0 
                            tmpVec[1] = 0.5*ΨLL(iLL,xnum)
                            tmpVec[2] = 0.0  # x0.5 is because iα = 1,2 summs up twice
                        else
                            tmpVec[1] = ΨLL(iLL,xnum)* (-1im)^(iLL)/sqrt(2)
                            tmpVec[2] = -1im * (2iα-3)*ΨLL(iLL-1,xnum)* (-1im)^(iLL-1) / sqrt(2) 
                        end
        
                        for iγ in 1:2
                            tmp = (Wγ[:,ilayer,z1,z2,iγ,ik1bar,ik2,ir]' * tmpVec) * 
                                    θ1  * θ3 * exp(-1im * s1 * real(Kl) * real(params.a1)) *
                                    ( sqrt(lB/real(params.a1)) / A.l1 ) 
