...
function initHofstadterVq(A::HofstadterVq, params::Params;p::Int=1,q::Int=5,lk::Int=4)
    ...
    A.n_ext = [1;sample(2:(A.l1*A.l2), min(A.l1*A.l2,9), replace=false)]
...
function computeSingleParticleSpectrum(A::HofstadterVq)
    ...
    A.H = zeros(ComplexF64,2A.q,2A.q,length(A.n_ext),length(A.ν))
    ...
    for ig2 in 1:size(A.gq,2)
        ...
        for ig1 in 1:size(A.gq,1) 
            for I_H in eachindex(A.n_ext)
                ...
                A.H[:,:,I_H,:] .+= reshape( sum(Vq .* OΨ1 .* conj.(OΨ2), dims=(4,5)), (2A.q, 2A.q,1))
                tmp = Coulomb(gq[ig1,ig2]) * (A.ρq[ig1,ig2] * (OΨ1[:,1,1,:,A.n_ext[I_H]])' )
                for iν in eachindex(A.ν)
                    A.H[:,:,I_H,iν] .+= A.ν[iν] * tmp
                end
            end
        end
    end

    # for iν in eachindex(A.ν), I_H in eachindex(A.n_ext)
    #     println(norm(A.H[:,:,I_H,iν]-A.H[:,:,I_H,iν]'))
    # end
